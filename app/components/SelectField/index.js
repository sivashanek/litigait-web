...
export default function ({ input, label, required, metaData, options, meta: { touched, error, warning } }) {
            ...
            <FormControl className={classes.formControl}>
                ...
                <Select
                    ...
                    {isPreDefinedSet ? (options || []).map((opt, index) => <MenuItem key={index} value={opt && opt.value || opt}>{opt && opt.label || opt}</MenuItem>) :
                        (metaData[options] || []).map((opt, index) => <MenuItem key={index} value={opt && opt.value != null
